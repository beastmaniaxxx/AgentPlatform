...
検索はテキスト→画像・画像→画像・画像→テキストのクロスモーダルに対応し、Reranking（関連度順の並べ替え）により関連度の高い結果を上位に提示する。事前にナレッジベースへ登録された画像（JPG/PNG/GIF、最大2MB）を検索対象とする。

> 実装方針に関する注記（Phase 0互換性スパイクの結果を反映）: Dify v1.11 のマルチモーダル埋め込みはクラウドプラグイン限定であり、ローカル・マルチモーダル埋め込み（Xinference）は非互換と確定した（`research.md` 参照）。本Specはプライバシー最優先方針（steering NFR#1）を優先し、**2系統のローカル手法を併用**して「画像→画像」を成立させる: (1) **ハッシュ照合**（コンテンツハッシュ＝完全一致、知覚ハッシュ＝視覚的な準一致）による視覚的一致検出、(2) **画像キャプション（説明文）をテキストKBに索引する caption 方式**による意味的関連検索。完全一致・視覚酷似はハッシュ照合で高精度に、内容が関連する画像はキャプション意味検索でカバーする。学習済みマルチモーダル埋め込みによる「異なるが視覚的に似た画像」の厳密な類似は本Specの対象外とする（将来Dify対応時に再検討）。いずれもローカル完結で外部送信を伴わない。
...
2. When ユーザーが画像を入力して検索を実行した場合, the マルチモーダルRAG機能 shall 当該画像の内容に関連する画像をナレッジベースから検索して取得する（関連性は画像内容の説明に基づく）
...
5. When ユーザーが画像を入力して検索を実行し、ナレッジベース内に当該画像と完全一致または視覚的に酷似する画像が存在する場合, the マルチモーダルRAG機能 shall それらの一致画像を検出し、他の関連結果より優先して提示する
...
1. When 検索で複数の候補結果が取得された場合, the マルチモーダルRAG機能 shall Reranking（関連度による並べ替え）により関連度の高い結果が上位になるよう並べ替える
