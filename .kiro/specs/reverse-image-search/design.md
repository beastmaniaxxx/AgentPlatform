...
本機能は、`dify-integration` Specが構築したPipeline中継基盤の上に、Open WebUIにアップロードされた画像を起点とする逆画像検索フローを追加する。Pipelineがアップロード画像（base64）を新規サービス imgpush へ変換して公開URLを取得し、その公開URLをクエリとしてDifyワークフロー（`reverse_image_search.yml`）へ中継する。ワークフローはSerpAPI（Google Lens）で類似画像を検索し、サムネイル（Markdown画像埋め込み）と出典サイトをローカルLLMで要約してチャットへ返す。画像が外部へ送信される旨は、Pipelineが情報通知（非ブロッキング）として応答に前置する。
...
### Goals
- アップロード画像（base64）を imgpush 経由で公開URL化し、SerpAPI（Google Lens）で逆画像検索する経路を確立する
...
- `workflows/reverse_image_search.yml`: SerpAPI（Google Lens）呼び出し→結果解析→0件分岐→LLM要約/Answer のDify advanced-chat DSL
...
- 外部サービス SerpAPI（Google Lens エンジン）
...
- `reverse_image_search.yml` の入力契約（クエリ＝公開画像URL）・出力フォーマット（Markdown画像＋出典）・SerpAPIエンジン/パラメータの変更 → ワークフローをインポートするオペレーター手順、および下流の `multimodal-rag`（結果フォーマット参照）で再確認
...
| 外部API | SerpAPI Google Lens エンジン | 公開画像URLから類似画像（visual_matches）を取得 | キーはDify Secret環境変数 `SERPAPI_KEY` |
...
docs/
└── reverse-image-search-setup.md     # 新規: imgpush公開設定・ワークフローインポート・SERPAPI_KEY/モデル設定・E2E確認手順
```
...
sequenceDiagram
    ...
    participant SerpAPI as SerpAPI Google Lens
    ...
    alt 画像が無い
        ...
        alt アップロード失敗
            ...
            DifyApi->>SerpAPI: google_lens url=公開URL (ssrf-proxy経由)
            SerpAPI-->>DifyApi: visual_matches
...
```mermaid
graph LR
    Start[Start sys.query equals public url] --> Http[HTTP Request SerpAPI google_lens]
    Http --> Parse[Code parse visual_matches]
    Parse --> Check{If-Else count equals 0}
...
**フロー上の決定事項**:
- HTTPノードはSerpAPIのクエリパラメータを `params`（`engine=google_lens`, `url={{#sys.query#}}`, `api_key={{#env.SERPAPI_KEY#}}`）で構成し、URL符号化を担保する。
- Codeノードは `visual_matches` から上位N件（既定5件程度）の `thumbnail`/`title`/`link`/`source` を抽出し、`![](thumbnail)` 形式のMarkdownと件数を算出。`thumbnail` 欠落の項目はスキップ。SerpAPIの `error` フィールド検出時は件数0相当として扱う。
- LLMノードは抽出結果を日本語で要約し、各項目に出典リンク（`[出典: source](link)`）を付与。
...
| ReverseImageSearch Workflow (`workflows/reverse_image_search.yml`) | Workflow | SerpAPI検索→解析→0件分岐→LLM要約/Answer | 1.2, 2.1, 2.2, 2.3, 4.2, 4.3 | SerpAPI (P0), Ollamaモデル (P1), dify-ssrf-proxy (P1) | API |
| imgpush Service | Infrastructure | base64画像の受領と公開配信 | 1.1 | agentplatform-net (P0) | State |
| Setup Documentation (`docs/reverse-image-search-setup.md`) | Documentation | 公開設定・インポート・Secret/モデル設定・E2E確認手順 | 1.1-1.3, 2.1-2.3, 3.1, 4.1-4.3 | - | - |
...
| Field | Detail |
|-------|--------|
| Intent | 公開画像URLをSerpAPI（Google Lens）で検索し、類似画像サムネイル＋出典をLLM要約して返すDify advanced-chat ワークフロー |
| Requirements | 1.2, 2.1, 2.2, 2.3, 4.2, 4.3 |

**Responsibilities & Constraints**
- Start→HTTP Request（SerpAPI `engine=google_lens`, `url={{#sys.query#}}`, `api_key={{#env.SERPAPI_KEY#}}`）→Code（`visual_matches` から上位N件抽出・Markdown生成・件数算出・`error` 検出）→If-Else（0件分岐）→LLM（要約・出典付与）/Answer（0件通知）→Answer（Markdown画像＋出典＋要約）
...
- Outbound: SerpAPI Google Lens — `dify-ssrf-proxy` 経由 (P0)
...
|--------|----------|---------|----------|--------|
| GET | `https://serpapi.com/search`（HTTPノード, ssrf-proxy経由） | `engine=google_lens&url=<公開画像URL>&api_key=<secret>` | `{visual_matches: [{title, link, source, thumbnail, ...}]}` | 非2xx / `{error}`（上限到達等）→ Codeで件数0相当扱い |
...
**Implementation Notes**
- Integration: 本ワークフローのAPIキーは `DIFY_REVERSE_IMAGE_SEARCH_APP_API_KEY` としてPipelinesランタイムに設定。`SERPAPI_KEY` はDify管理画面でSecret環境変数として設定
- Validation: SerpAPIのモック/実応答に対するデバッグ実行で、1件以上時にMarkdown画像＋出典＋要約、0件時に再検索通知が返ること（要件2.1-2.3, 4.2）
- Risks: `dify-ssrf-proxy` 経由でのserpapi.com（HTTPS）到達、URLパラメータ符号化、SerpAPI応答スキーマ変化（`visual_matches` キー）。実装時に実応答で検証
...
- `reverse_image_search.yml` をインポート・SERPAPI_KEY設定後、SerpAPIモック/実応答に対するDifyデバッグ実行で、1件以上時にMarkdown画像＋出典＋要約、0件時に再検索通知が返ること（要件2.1-2.3, 4.2）
...
- `SERPAPI_KEY` はDify Secret環境変数として管理し、DSL・`docker/.env` に平文で置かない（research.md 設計判断参照）。`DIFY_REVERSE_IMAGE_SEARCH_APP_API_KEY` は `docker/.env`（VCS対象外）で管理
